...
def mem(nc, nnode, pm_factor= 3)
  ...
  np= part_alloc_factor*nc*nc*(nx+1)
  mp= part_alloc_factor*nc*nc*(nx+1)*sizep
  mb= nc*nc*(1)*sizep;
  mm= (pm_factor*nc)**2*pm_nx*4*2  # factor of 2 for FFTW working memory(?)
  mm2= (pm_factor*nc)**2*(nx/2+1)*2*4 # if delta_k is copied

  printf("%.1f Mbytes for %d particle\n", mp/mbyte, np)
  printf("%.1f Mbytes for communication buffer\n", mb/mbyte)
  printf("%.1f Mbytes for one PM mesh\n", mm/mbyte)

  printf("%.3f Gbytes per core in total\n", (mp + mb + mm + mm2)/gbyte)
